...
| RDS | RDS-03 | 저장 데이터 암호화 미적용 | MEDIUM |
| RDS | RDS-04 | 보안 그룹 과다 개방 | HIGH |
| RDS | RDS-05 | 자동 백업 보존 기간 | LOW |
| RDS | RDS-06 | SSL/TLS 전송 암호화 | LOW |
| RDS | RDS-07 | CloudWatch 로그 설정 | LOW |
| S3 | S3-01 | 퍼블릭 액세스 차단 설정 | HIGH |
| S3 | S3-02 | 버킷 정책 퍼블릭 허용 | HIGH |
| S3 | S3-03 | 서버 측 암호화(SSE) 미적용 | MEDIUM |
| S3 | S3-04 | 버전 관리 비활성화 | LOW |
| S3 | S3-05 | 서버 액세스 로깅 미설정 | LOW |
| S3 | S3-06 | ACL 퍼블릭 허용 | HIGH |
| IAM | IAM-01 | 루트 계정 일상적 사용 | MEDIUM |
| IAM | IAM-02 | 액세스 키 장기 미교체 (90일+) | HIGH |
| IAM | IAM-03 | 사용자 MFA 미설정 | MEDIUM |
...
| EC2 | EC2-03 | EBS 볼륨 암호화 누락 | MEDIUM |
| EC2 | EC2-04 | IAM 역할 미할당 | HIGH |
| EC2 | EC2-05 | User Data 내 민감 정보 포함 | MEDIUM |
| CloudTrail | CT-01 | 조직 추적(Organization Trail) 미설정 | LOW |
| CloudTrail | CT-02 | S3 Object Lock(WORM) 미설정 | LOW |
| CloudTrail | CT-03 | 데이터 이벤트 추적 누락 | LOW |
| CloudTrail | CT-04 | CloudWatch Logs 연동 누락 | LOW |
| CloudTrail | CT-05 | 단일 리전 설정 | LOW |
| CloudWatch | CW-01 | 보안 이벤트 지표 필터·경보 누락 | LOW |
| CloudWatch | CW-02 | 로그 그룹 KMS 암호화 미적용 | LOW |
| CloudWatch | CW-03 | 로그 그룹 보존 주기 무제한 | LOW |
| CloudWatch | CW-04 | CloudTrail → CloudWatch 연동 누락 | LOW |
| CloudWatch | CW-05 | VPC Flow Logs 수집 누락 | LOW |
| VPC | VPC-01 | 라우팅 테이블 IGW 경로 누락 | LOW |
| VPC | VPC-02 | 퍼블릭 IP 자동 할당 비활성화 | MEDIUM |
| VPC | VPC-03 | NAT 게이트웨이 프라이빗 서브넷 오배치 | LOW |
| VPC | VPC-04 | NACL 아웃바운드 전체 차단 | LOW |

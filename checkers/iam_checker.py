...
class IAMChecker(BaseChecker):
    ...
    def _check_root_usage(self, iam):
        try:
            ...
            if not root:
                return self._make_result("IAM-01","루트 계정 일상적 사용",HIGH,"root",
                    WARN,"루트 계정 정보를 찾을 수 없음","credential report 권한 확인 필요")
            ...
            if recent:
                return self._make_result("IAM-01","루트 계정 일상적 사용",HIGH,"root",
                    FAIL,"루트 계정 최근 사용 감지: "+", ".join(recent),
                    "IAM 관리자 계정 생성 후 루트 계정 봉인 권장 (MFA 설정 필수)")
            return self._make_result("IAM-01","루트 계정 일상적 사용",HIGH,"root",
                PASS,"최근 90일 내 루트 계정 사용 기록 없음")
        except Exception as e:
            return self._make_result("IAM-01","루트 계정 일상적 사용",HIGH,"root",
                WARN,f"조회 실패: {e}","iam:GenerateCredentialReport 권한 확인")
        ...
        try:
            ...
            for key in keys:
                ...
                if key["Status"] == "Inactive":
                    results.append(self._make_result("IAM-02","비활성 액세스 키 방치",MEDIUM,rid,
                        WARN,f"비활성 키 존재 (생성 {days}일 전) — 미사용 시 삭제 권장",
                        f"aws iam delete-access-key --user-name {username} --access-key-id {kid}"))
                elif days > self.STALE_KEY_DAYS:
                    results.append(self._make_result("IAM-02","액세스 키 장기 미교체",HIGH,rid,
                        FAIL,f"활성 키 {days}일째 미교체 (90일 초과) — 코드 내 하드코딩 위험",
                        f"새 키 발급 후 기존 키 삭제 (aws iam create-access-key / delete-access-key)"))
                else:
                    results.append(self._make_result("IAM-02","액세스 키 장기 미교체",HIGH,rid,
                        PASS,f"활성 키 생성 {days}일 경과 (90일 이내)"))
        except Exception as e:
            results.append(self._make_result("IAM-02","액세스 키 장기 미교체",HIGH,username,
                WARN,f"조회 실패: {e}","iam:ListAccessKeys 권한 확인"))
        return results
    ...
    def _check_mfa(self, iam, username):
        try:
            ...
            if not devices:
                return self._make_result("IAM-03","사용자 MFA 미설정",HIGH,username,
                    FAIL,"MFA 미등록 — 비밀번호 유출 시 방어 수단 없음",
                    "AWS 콘솔 → IAM → 사용자 → 보안 자격 증명 → MFA 디바이스 할당")
            return self._make_result("IAM-03","사용자 MFA 미설정",HIGH,username,
                PASS,f"MFA 기기 {len(devices)}개 등록됨")
        except Exception as e:
            return self._make_result("IAM-03","사용자 MFA 미설정",HIGH,username,
                WARN,f"조회 실패: {e}","iam:ListMFADevices 권한 확인")
        ...
        try:
            for policy in iam.list_attached_user_policies(UserName=username).get("AttachedPolicies",[]):
                ...
                try:
                    ...
                    if vulns:
                        results.append(self._make_result("IAM-04","와일드카드(*) 과도한 권한",HIGH,
                            f"{username}/{pname}",FAIL,"와일드카드 남용: "+" ".join(vulns),
                            "정책 편집 → 필요한 Action/Resource만 명시 (예: s3:GetObject, arn:aws:s3:::bucket/*)"))
                    else:
                        results.append(self._make_result("IAM-04","와일드카드(*) 과도한 권한",HIGH,
                            f"{username}/{pname}",PASS,"와일드카드 남용 없음"))
                except Exception:
                    pass
        except Exception as e:
            results.append(self._make_result("IAM-04","와일드카드(*) 과도한 권한",HIGH,username,
                WARN,f"조회 실패: {e}","iam:ListAttachedUserPolicies 권한 확인"))
        return results
    ...
    def _check_direct_policy(self, iam, username):
        try:
            ...
            if issues:
                return self._make_result("IAM-05","사용자 직접 정책 연결",LOW,username,
                    WARN,"; ".join(issues),
                    "IAM 그룹 생성 → 그룹에 정책 부여 → 사용자를 그룹에 추가 후 직접 연결 정책 제거")
            return self._make_result("IAM-05","사용자 직접 정책 연결",LOW,username,
                PASS,"직접 연결 정책 없음")
        except Exception as e:
            return self._make_result("IAM-05","사용자 직접 정책 연결",LOW,username,
                WARN,f"조회 실패: {e}","iam:ListUserPolicies 권한 확인")

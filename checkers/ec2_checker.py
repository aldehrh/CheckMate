            ...
            for sg in sgs:
                ...
                if open_rules:
                    results.append(self._make_result(
                        "EC2-01", "보안 그룹 인바운드 과다 허용", HIGH, sg_name,
                        FAIL,
                        f"0.0.0.0/0 허용 규칙 {len(open_rules)}개: " + " | ".join(open_rules),
                        f"EC2 → 보안 그룹 → {sg_id} → 인바운드 규칙 편집\n"
                        f"→ 0.0.0.0/0 규칙 삭제 후 특정 IP 또는 보안 그룹으로 재제한",
                    ))
                else:
                    results.append(self._make_result(
                        "EC2-01", "보안 그룹 인바운드 과다 허용", HIGH, sg_name,
                        PASS, "0.0.0.0/0 허용 규칙 없음",
                    ))
        except Exception as e:
            results.append(self._make_result(
                "EC2-01", "보안 그룹 인바운드 과다 허용", HIGH, "security-groups",
                WARN, f"조회 실패: {e}", "ec2:DescribeSecurityGroups 권한 확인",
            ))
        return results
            ...
            for res in reservations:
                for inst in res.get("Instances", []):
                    ...
                    if http_tokens != "required":
                        results.append(self._make_result(
                            "EC2-02", "IMDSv2 미강제", HIGH, inst_id,
                            FAIL,
                            f"HttpTokens={http_tokens} → IMDSv1 허용 상태 (SSRF 공격으로 자격 증명 탈취 위험)",
                            f"EC2 → 인스턴스 선택 → 작업 → 인스턴스 설정 → 인스턴스 메타데이터 옵션 수정\n"
                            f"→ IMDSv2를 '필수'로 변경\n"
                            f"(CLI) aws ec2 modify-instance-metadata-options "
                            f"--instance-id {inst_id} --http-tokens required",
                        ))
                    else:
                        results.append(self._make_result(
                            "EC2-02", "IMDSv2 미강제", HIGH, inst_id,
                            PASS, f"HttpTokens=required (IMDSv2 강제 적용 중)",
                        ))
        except Exception as e:
            results.append(self._make_result(
                "EC2-02", "IMDSv2 미강제", HIGH, "instances",
                WARN, f"조회 실패: {e}", "ec2:DescribeInstances 권한 확인",
            ))
        return results
    ...
    def _check_ebs_encryption(self, ec2) -> list:
        ...
        try:
            ...
            for vol in volumes:
                ...
                if not encrypted:
                    results.append(self._make_result(
                        "EC2-03", "EBS 볼륨 암호화 누락", HIGH, vol_id,
                        FAIL,
                        f"암호화 미적용 (상태: {state}, 크기: {size}GB) — 스토리지 침해 시 데이터 즉시 노출",
                        f"기존 볼륨 소급 불가 → 스냅샷 생성 후 암호화 복사:\n"
                        f"aws ec2 create-snapshot --volume-id {vol_id}\n"
                        f"aws ec2 copy-snapshot --source-snapshot-id <snap-id> --encrypted "
                        f"--kms-key-id alias/aws/ebs",
                    ))
                else:
                    results.append(self._make_result(
                        "EC2-03", "EBS 볼륨 암호화 누락", HIGH, vol_id,
                        PASS, f"암호화 적용됨 (상태: {state}, 크기: {size}GB)",
                    ))
        except Exception as e:
            results.append(self._make_result(
                "EC2-03", "EBS 볼륨 암호화 누락", HIGH, "volumes",
                WARN, f"조회 실패: {e}", "ec2:DescribeVolumes 권한 확인",
            ))
        return results
            ...
            for res in reservations:
                for inst in res.get("Instances", []):
                    ...
                    if "IamInstanceProfile" not in inst:
                        results.append(self._make_result(
                            "EC2-04", "IAM 역할 미할당", HIGH, f"{inst_id} ({name})",
                            FAIL,
                            "IamInstanceProfile 없음 → 액세스 키를 인스턴스 내부에 직접 저장할 위험",
                            f"EC2 → 인스턴스 선택 → 작업 → 보안 → IAM 역할 수정\n"
                            f"→ 적절한 권한의 역할 선택 후 저장",
                        ))
                    else:
                        profile = inst["IamInstanceProfile"].get("Arn", "?")
                        results.append(self._make_result(
                            "EC2-04", "IAM 역할 미할당", HIGH, f"{inst_id} ({name})",
                            PASS, f"IAM 역할 할당됨: {profile}",
                        ))
        except Exception as e:
            results.append(self._make_result(
                "EC2-04", "IAM 역할 미할당", HIGH, "instances",
                WARN, f"조회 실패: {e}", "ec2:DescribeInstances 권한 확인",
            ))
        return results
        ...
        try:
            ...
            for inst_id in instance_ids:
                try:
                    ...
                    if not value:
                        results.append(self._make_result(
                            "EC2-05", "User Data 민감 정보", MEDIUM, inst_id,
                            PASS, "User Data 없음",
                        ))
                        continue
                    ...
                    if found:
                        results.append(self._make_result(
                            "EC2-05", "User Data 민감 정보", MEDIUM, inst_id,
                            FAIL,
                            f"민감 키워드 발견: {', '.join(found)}\n미리보기: {snippet}",
                            "User Data에서 민감 정보 제거:\n"
                    ...
                    results.append(self._make_result(
                        "EC2-05", "User Data 민감 정보", MEDIUM, inst_id,
                        WARN, f"User Data 조회 실패: {e}",
                        "ec2:DescribeInstanceAttribute 권한 확인",
                    ))
        except Exception as e:
            results.append(self._make_result(
                "EC2-05", "User Data 민감 정보", MEDIUM, "instances",
                WARN, f"조회 실패: {e}", "ec2:DescribeInstances 권한 확인",
            ))
        return results

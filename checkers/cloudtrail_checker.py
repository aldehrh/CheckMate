...
class CloudTrailChecker(BaseChecker):
    ...
    def _check_org_trail(self, trail):
        ...
        return self._make_result(
            "CT-01", "조직 추적(Organization Trail) 미설정", MEDIUM, name,
            PASS if is_org else WARN,
            f"IsOrganizationTrail={is_org}" + (" — 하위 계정 이벤트 통합 수집 중" if is_org
             else " — 하위 계정 이벤트 누락 가능"),
            "CloudTrail → 추적 선택 → 편집 → '조직의 모든 계정에 대해 활성화' 체크"
            if not is_org else "조치 불필요"
        )
    ...
    def _check_s3_object_lock(self, trail, s3):
        ...
        if not bucket:
            return self._make_result(
                "CT-02", "S3 Object Lock(WORM) 미설정", MEDIUM, name,
                WARN, "연결된 S3 버킷 정보 없음", "Trail에 S3 버킷 연결 확인"
            )
        try:
            ...
            return self._make_result(
                "CT-02", "S3 Object Lock(WORM) 미설정", MEDIUM, f"{name} → s3://{bucket}",
                PASS if enabled else WARN,
                f"ObjectLockEnabled={enabled}, Mode={mode}",
                f"S3 → {bucket} → 속성 → 객체 잠금 → 기본 보존 활성화 (거버넌스/규정준수)"
                if not enabled else "조치 불필요"
            )
        except s3.exceptions.NoSuchObjectLockConfiguration:
            return self._make_result(
                "CT-02", "S3 Object Lock(WORM) 미설정", MEDIUM, f"{name} → s3://{bucket}",
                WARN, "Object Lock 설정 없음 — 관리자 권한 탈취 시 로그 영구 삭제 가능",
                f"버킷 생성 시에만 Object Lock 활성화 가능. 신규 버킷에 적용 후 Trail 재연결 권장"
            )
        except Exception as e:
            return self._make_result(
                "CT-02", "S3 Object Lock(WORM) 미설정", MEDIUM, f"{name} → s3://{bucket}",
                WARN, f"조회 실패: {e}", "s3:GetObjectLockConfiguration 권한 확인"
            )
    ...
    def _check_data_events(self, trail, ct):
        ...
        try:
            ...
            return self._make_result(
                "CT-03", "데이터 이벤트 추적 누락", HIGH, name,
                PASS if has_data else WARN,
                "데이터 이벤트 추적 활성화됨" if has_data
                else "데이터 이벤트 미설정 — S3 객체 접근·Lambda 호출 이력 누락",
                "CloudTrail → 추적 선택 → 데이터 이벤트 편집 → S3/Lambda 데이터 이벤트 활성화"
                if not has_data else "조치 불필요"
            )
        except Exception as e:
            return self._make_result(
                "CT-03", "데이터 이벤트 추적 누락", HIGH, name,
                WARN, f"조회 실패: {e}", "cloudtrail:GetEventSelectors 권한 확인"
            )
    ...
    def _check_cloudwatch_integration(self, trail):
        ...
        if log_arn:
            return self._make_result(
                "CT-04", "실시간 경고 체계 미설정", MEDIUM, name,
                PASS, f"CloudWatch Logs 연동됨: {log_arn}"
            )
        return self._make_result(
            "CT-04", "실시간 경고 체계 미설정", MEDIUM, name,
            FAIL, "CloudWatchLogsLogGroupArn 없음 — S3 저장만 되어 실시간 탐지 불가",
            "CloudTrail → 추적 선택 → CloudWatch Logs 편집 → 활성화 + 로그 그룹·IAM 역할 지정"
        )
    ...
    def _check_multi_region(self, trail):
        ...
        return self._make_result(
            "CT-05", "단일 리전 설정", MEDIUM, name,
            PASS if is_multi else WARN,
            f"IsMultiRegionTrail={is_multi}" + (" — 전체 리전 API 활동 수집 중" if is_multi
             else " — 미사용 리전에서의 침입 활동 탐지 불가"),
            "CloudTrail → 추적 선택 → 편집 → '모든 리전에 추적 적용' 체크"
            if not is_multi else "조치 불필요"
        )
